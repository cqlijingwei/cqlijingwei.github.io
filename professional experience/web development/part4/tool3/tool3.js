...
function addCourses() {
	...
	for (i=0;i<numOfCourse;i++) {
		...
		input1.name = i;
		input1.id = "courseGP";
		input1.onkeypress=numberOnly(event, this);
		input1.onkeyup=getGPA();
		input1.onfocus=this.select();
		...
		input2.name = i;
		input2.id = "courseWei";
		input2.onkeypress=numberOnly(event, this);
		input2.onkeyup=getGPA();
		input2.onfocus=this.select();
