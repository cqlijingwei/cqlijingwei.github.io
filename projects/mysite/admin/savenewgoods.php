...
if(is_numeric($_POST['shichangjia'])==false || is_numeric($_POST['huiyuanjia'])==false)
 {
   echo "<script>alert('Price can only be a number£¡');history.back();</script>";
   exit;
 }
if(is_numeric($_POST['shuliang'])==false)
 {
   echo "<script>alert('The number can only be a number£¡');history.back();</script>";
   exit;
 }
...
echo "<script>alert('Commodity".$mingcheng."Added successfully!');window.location.href='addgoods.php';</script>";
?>